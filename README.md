### Profile

Technical Designer & Creative Technologist with a broad practical skillset.

#### Technical Skills

* **Technical Design** Rhino 3D, AutoCAD, Solidworks, F360
* **CG** Blender 3D, Zbrush, Keyshot, Octane
* **Art** Oil Painting, Sketching, Drawing
* **Printmaking** Intaglio, Letterpress, Woodblock, Lithography
* **Media** Videography, Streaming, Editing
* **Scripting** Python, Lisp, Bash
* **Web Dev** Unix, Py, Go, NEXT.js, CSS grid
* **Electronics** Raspberry Pi, Arduino
* **UI/UX** Adobe XD, Figma
* **Graphics** Inkscape, Krita, PS, AI
* **Mograph** After Effects, Blender
* **Fabrication** Woodworking, Metalwork, CNC, 3D printing

#### Professional Experience

**Assistant Art Director** Directors Guild of Ontario - Various Projects 2018-Current

**Adjunct Instructor** Loyalist College, OCADU, Online 2016-2018

**Freelance Technical Designer** 002478101 Ontario Inc. - Various Clients 2015-Current



